...
5.  **Output:** Return ONLY the final, edited image. Do not include any text.`;
    ...
    const accessoryPrompt = `You are an expert virtual try-on AI. You will be given a 'model image' and a 'garment image' (which is an accessory). Your task is to create a new photorealistic image where the person from the 'model image' is wearing the accessory from the 'garment image'.
...
5.  **Output:** Return ONLY the final, edited image. Do not include any text.`;
    ...
    const prompt = garmentInfo.type === 'accessory' ? accessoryPrompt : garmentPrompt;
...
export const changeBackgroundImage = async (baseImageUrl: string, backgroundInstruction: string): Promise<string> => {
    const baseImagePart = dataUrlToPart(baseImageUrl);
    const prompt = `You are an expert photo editor AI. You will be given an image of a person. Your task is to meticulously replace the background of the image.

**Crucial Rules:**
1.  **New Background:** The new background must be: "${backgroundInstruction}".
