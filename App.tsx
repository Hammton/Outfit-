...
const App: React.FC = () => {
  ...
  const handleBackgroundChange = useCallback(async (backgroundInstruction: string) => {
    if (!displayImageUrl || isLoading) return;
    ...
    try {
      const newImageUrl = await changeBackgroundImage(displayImageUrl, backgroundInstruction);
      
      // Update the current image in the history
      setOutfitHistory(prevHistory => {
        const newHistory = [...prevHistory];
